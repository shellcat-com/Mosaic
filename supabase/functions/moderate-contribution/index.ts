  ...
  fetch: withSupabase<any>({ auth: "user" }, async (request, ctx) => {
    try {
      ...
      const decision = requiredString(
        input.evidenceDecision,
        "evidenceDecision",
        10,
      );
      if (decision !== "approved" && decision !== "rejected") {
        throw new HttpError(400, "Invalid evidence decision");
      }
      if (
        input.memoryDecision &&
        !["approved", "rejected"].includes(input.memoryDecision)
      ) {
      ...
      const { data: contribution, error: contributionError } = await admin.from(
        ...
        .select("id,challenge_id,status,evidence_method").eq(
          "id",
          contributionId,
        ).single();
      ...
      if (
        !["pending_review", "verified", "rejected"].includes(
          contribution.status,
        )
      ) {
      ...
      const { data: updated, error: updateError } = await admin.from(
        ...
        verification_level: decision === "approved"
          ? "organizer_verified"
          : null,
        updated_at: now,
      ...
      const memoryDecision = decision === "rejected"
        ? "rejected"
        : input.memoryDecision;
      if (memoryDecision) {
        let memoryMediaPath: string | null = null;
        if (memoryDecision === "approved") {
          const { data: evidence, error: evidenceLookupError } = await admin
            .from("evidence_submissions")
            .select("media_path,mime_type").eq(
              "contribution_id",
              contributionId,
            ).single();
          ...
          if (evidence.media_path) {
            ...
            memoryMediaPath =
              `${contribution.challenge_id}/${contributionId}.${extension}`;
      ...
      const { error: auditError } = await admin.from("moderation_actions")
        .upsert({
