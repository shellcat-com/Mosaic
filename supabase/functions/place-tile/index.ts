...
import { body, failure, HttpError, ok, requiredUUID, userId } from "../_shared/mosaic.ts"
...
export default {
  fetch: withSupabase({ auth: "user" }, async (request, ctx) => {
    try {
      ...
      const { data, error } = await ctx.supabaseAdmin.rpc("internal_place_tile", {
        target_contribution_id: contributionId,
        target_user_id: uid,
      })
      if (error) {
        if (error.message.includes("not ready") || error.message.includes("not owned")) {
