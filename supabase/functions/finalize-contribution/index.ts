...
export default {
  fetch: withSupabase<any>({ auth: "user" }, async (request, ctx) => {
    try {
      ...
      const { data: owner, error: ownerError } = await admin.from(
        "contribution_owners",
      )
        .select("participant_id").eq("contribution_id", contributionId)
        .maybeSingle();
      if (ownerError) throw ownerError;
      if (!owner || owner.participant_id !== uid) {
        throw new HttpError(403, "Only the contributor can finalize evidence");
      }

      const { data: contribution, error: contributionError } = await admin.from(
        "contributions",
      )
        .select(
          "id,challenge_id,mission_id,emotion,evidence_method,status,verification_level,tile_position",
        )
        .eq("id", contributionId).single();
      if (contributionError) throw contributionError;
      if (contribution.status !== "draft") return ok({ contribution });

      const { data: evidence, error: evidenceError } = await admin.from(
        "evidence_submissions",
      )
        .select("media_path,reflection_text,mime_type").eq(
          "contribution_id",
          contributionId,
        ).single();
      ...
      if (
        ["photo", "video", "receipt"].includes(contribution.evidence_method)
      ) {
        ...
        const { data: objects, error: listError } = await admin.storage
          .from("evidence-private").list(folder, {
            search: filename,
            limit: 1,
          });
      ...
      const { error: ownerUpdateError } = await admin.from(
        "contribution_owners",
      ).update({
      ...
      const { error: evidenceUpdateError } = await admin.from(
        "evidence_submissions",
      ).update({
      ...
      if (input.includeMemory) {
        const storyText = optionalString(input.storyText, "storyText") ??
          evidence.reflection_text;
        let memoryPath: string | null = null;
        if (evidence.media_path && evidence.mime_type?.startsWith("image/")) {
          ...
          memoryPath =
            `${contribution.challenge_id}/${contributionId}/memory.${extension}`;
          const { error: memoryUploadError } = await admin.storage.from(
            "recap-memories",
          ).upload(
            memoryPath,
            downloaded,
