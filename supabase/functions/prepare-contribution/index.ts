  ...
  fetch: withSupabase<any>({ auth: "user" }, async (request, ctx) => {
    try {
      ...
      if (!allowedEmotions.has(emotion)) {
        throw new HttpError(400, "emotion is invalid");
      }
      if (!allowedEvidence.has(method)) {
        throw new HttpError(400, "evidence method is unavailable");
      }
      await assertMember(ctx, challengeId);

      const { data: challenge, error: challengeError } = await ctx.supabase
        .from("challenges").select("is_showcase,status").eq("id", challengeId)
        .single();
      if (challengeError) throw challengeError;
      if (challenge.is_showcase) {
        throw new HttpError(
          403,
          "The showcase is read-only; use the organizer sandbox",
        );
      }
      if (challenge.status !== "active") {
        throw new HttpError(
          409,
          "This challenge is no longer accepting evidence",
        );
      }

      const { data: mission, error: missionError } = await ctx.supabase
        .from("missions").select("accepted_evidence").eq("id", missionId).eq(
          "challenge_id",
          challengeId,
        ).single();
      if (missionError) throw missionError;
      if (!mission.accepted_evidence.includes(method)) {
        throw new HttpError(400, "Mission does not accept this evidence");
      }
      ...
      if (["photo", "video", "receipt"].includes(method)) {
        ...
        if (!Number.isInteger(fileSize) || Number(fileSize) <= 0) {
        ...
        const isVideo = mimeType === "video/quicktime" ||
          mimeType === "video/mp4";
        if (
          (method === "video" && !isVideo) || (method !== "video" && !isImage)
        ) {
          throw new HttpError(
            400,
            "Media type does not match the evidence method",
          );
        }
        if (method === "video") {
          if (Number(fileSize) > 26_214_400) {
        ...
        mediaPath = `${challengeId}/${uid}/${contributionId}.${
      ...
      if (mediaPath) {
        const { data: signed, error: signedError } = await admin.storage
          .from("evidence-private").createSignedUploadUrl(mediaPath, {
            upsert: true,
          });
