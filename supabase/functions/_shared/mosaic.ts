...
export function requiredString(value: unknown, field: string, max = 500): string {
  ...
  if (!normalized || normalized.length > max) throw new HttpError(400, `${field} is invalid`)
  return normalized
}
...
export async function assertOrganizer(ctx: FunctionContext, challengeId: string) {
  const membership = await assertMember(ctx, challengeId)
  if (membership.role !== "organizer") throw new HttpError(403, "Organizer access required")
  return membership
}
...
export const allowedEvidence = new Set(["reflection", "photo", "video", "receipt", "organizer"])
...
export function extensionForMime(mimeType: string): string {
  switch (mimeType) {
    ...
    default: throw new HttpError(400, "Unsupported media type")
  }
}
