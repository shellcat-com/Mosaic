...
import { body, failure, HttpError, ok, requiredString, requiredUUID } from "../_shared/mosaic.ts"
...
export default {
  fetch: withSupabase({ auth: "user" }, async (request, ctx) => {
    try {
      ...
      if (kind !== "evidence" && kind !== "memory") throw new HttpError(400, "kind is invalid")

      const table = kind === "evidence" ? "evidence_submissions" : "memories"
      const { data, error } = await ctx.supabase.from(table)
        .select("media_path").eq("contribution_id", contributionId).maybeSingle()
      if (error) throw error
      if (!data) throw new HttpError(403, "Media is not available to this user")
      if (!data.media_path) throw new HttpError(404, "This item has no media")

      const bucket = kind === "evidence" ? "evidence-private" : "memory-private"
