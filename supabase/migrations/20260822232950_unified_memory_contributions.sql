-- Unify recap memories around three explicit forms: photo, short video, and note.
alter table public.shared_moments
  add column media_kind text not null default 'photo',
  add column media_mime_type text,
  add column duration_seconds numeric(5,2);

update public.shared_moments
set media_kind = case
  when media_path is null then 'note'
  when lower(media_path) like '%.mov' or lower(media_path) like '%.mp4' then 'video'
  else 'photo'
end,
media_mime_type = case
  when lower(coalesce(media_path, '')) like '%.mov' then 'video/quicktime'
  when lower(coalesce(media_path, '')) like '%.mp4' then 'video/mp4'
  when media_path is not null then 'image/jpeg'
  else null
end;

alter table public.shared_moments
  add constraint shared_moments_media_kind check (media_kind in ('photo', 'video', 'note')),
  add constraint shared_moments_video_duration check (
    duration_seconds is null or duration_seconds between 0 and 10.0
  ),
  add constraint shared_moments_content_present check (
    (media_kind = 'note' and nullif(btrim(note), '') is not null)
    or (media_kind in ('photo', 'video') and media_path is not null)
    or lifecycle = 'upload_pending'
  );

update storage.buckets
set file_size_limit = 26214400,
    allowed_mime_types = array['image/jpeg', 'image/png', 'video/quicktime', 'video/mp4']
where id = 'recap-memories';
...
select m.id, 'contribution'::text as origin, m.contribution_id, c.challenge_id, co.participant_id,
  ...
  case when m.media_path is null then 'note' else 'photo' end::text as media_kind,
  case when m.media_path is null then null else 'image/jpeg' end::text as media_mime_type,
