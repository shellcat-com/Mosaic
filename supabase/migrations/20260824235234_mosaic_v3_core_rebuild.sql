-- Mosaic v3 is an intentionally destructive reset for the non-production backend.
-- Back up the linked project before applying. Migration history remains in
-- supabase_migrations; only the legacy product API in public is replaced.
...
select cron.unschedule(jobid) from cron.job where jobname like 'mosaic-%';

-- Dropping the product schemas cascades their Storage policies. Supabase
-- intentionally forbids SQL deletion from storage-owned tables; abandoned
-- legacy bucket objects must be purged through the Storage API during the
-- pre-deployment backup/reset procedure, not from migration SQL.
...
create table public.mosaics (
  ...
  artwork_id uuid not null references public.artwork_catalog(id),
...
create unique index mosaic_single_creator on public.mosaic_members(mosaic_id) where role = 'creator';

create table public.kindness_activities (
  id uuid primary key default gen_random_uuid(),
  mosaic_id uuid not null references public.mosaics(id) on delete cascade,
...
create table public.kindness_contributions (
  id uuid primary key default gen_random_uuid(),
  mosaic_id uuid not null references public.mosaics(id) on delete cascade,
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  foreign key (activity_id, mosaic_id) references public.kindness_activities(id, mosaic_id) on delete cascade,
...
create index event_photos_gallery_idx on public.event_photos(mosaic_id, captured_at) where state = 'eligible';
create index event_photos_shot_count_idx on public.event_photos(mosaic_id, photographer_id) where state in ('upload_pending', 'eligible');
...
create table private.artwork_reveal_packages (
  mosaic_id uuid primary key references public.mosaics(id) on delete cascade,
...
insert into public.artwork_catalog (id, title, artist, asset_name, source_url, license, alt_text, reviewed_at) values
...
create or replace function private.is_mosaic_member(target_mosaic uuid, target_user uuid default auth.uid())
returns boolean language sql stable security definer set search_path = '' as $$
  select target_user is not null and exists (
    select 1 from public.mosaic_members mm where mm.mosaic_id = target_mosaic and mm.profile_id = target_user
  );
$$;

create or replace function private.is_mosaic_creator(target_mosaic uuid, target_user uuid default auth.uid())
returns boolean language sql stable security definer set search_path = '' as $$
  select target_user is not null and exists (
    select 1 from public.mosaics m where m.id = target_mosaic and m.creator_id = target_user
  );
$$;

create or replace function private.mosaic_revealed(target_mosaic uuid)
returns boolean language sql stable security definer set search_path = '' as $$
  select exists (select 1 from public.mosaics m where m.id = target_mosaic and now() >= m.reveal_at);
$$;

create or replace function private.artwork_json(art public.artwork_catalog)
...
create or replace function private.sealed_artwork_json()
...
create or replace function private.summary_json(m public.mosaics)
returns jsonb language sql stable security definer set search_path = '' as $$
  select jsonb_build_object(
    ...
    'contributionCount', (select count(*) from public.kindness_contributions c where c.mosaic_id = m.id),
    'photoCount', (select count(*) from public.event_photos p where p.mosaic_id = m.id and p.state = 'eligible'),
    'filmLookID', m.film_look_id, 'shotLimit', m.shot_limit,
    'artwork', case when now() >= m.reveal_at then private.artwork_json(a) else private.sealed_artwork_json() end,
    'isCreator', m.creator_id = auth.uid()
  ) from public.artwork_catalog a where a.id = m.artwork_id;
$$;

create or replace function private.event_json(m public.mosaics)
returns jsonb language sql stable security definer set search_path = '' as $$
  select private.summary_json(m) || jsonb_build_object(
    ...
    'activities', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'participantCompleted', exists(select 1 from public.kindness_contributions mine where mine.activity_id = a.id and mine.participant_id = auth.uid())
      ) order by a.sort_order) from public.kindness_activities a where a.mosaic_id = m.id
    ), '[]'::jsonb),
    'contributions', coalesce((
      ...
      from public.kindness_contributions c join public.profiles p on p.id = c.participant_id
      where c.mosaic_id = m.id and (now() >= m.reveal_at or c.participant_id = auth.uid())
    ), '[]'::jsonb),
    'contributionCount', (select count(*) from public.kindness_contributions c where c.mosaic_id = m.id),
    'occupiedTilePositions', coalesce((select jsonb_agg(c.tile_position order by c.tile_position) from public.kindness_contributions c where c.mosaic_id = m.id), '[]'::jsonb),
    'photos', coalesce((
      select jsonb_agg(jsonb_build_object(
      ...
      from public.event_photos ep join public.profiles pp on pp.id = ep.photographer_id
      where ep.mosaic_id = m.id and ep.state = 'eligible'
        and (ep.photographer_id = auth.uid() or now() >= m.reveal_at)
        and not exists (select 1 from public.user_blocks b where b.blocker_id = auth.uid() and b.blocked_id = ep.photographer_id)
    ), '[]'::jsonb),
    'memberCount', (select count(*) from public.mosaic_members mm where mm.mosaic_id = m.id),
    'isCreator', m.creator_id = auth.uid()
  );
$$;

revoke all on all functions in schema private from public, anon, authenticated;
grant execute on function private.is_mosaic_member(uuid,uuid) to authenticated;
grant execute on function private.is_mosaic_creator(uuid,uuid) to authenticated;
grant execute on function private.mosaic_revealed(uuid) to authenticated;
grant execute on function private.summary_json(public.mosaics) to authenticated;
...
alter table public.kindness_activities enable row level security;
alter table public.kindness_contributions enable row level security;
...
grant select on public.artwork_catalog to anon, authenticated;
grant select on public.mosaics, public.mosaic_members, public.kindness_activities, public.kindness_contributions, public.event_photos, public.event_photo_reports, public.user_blocks to authenticated;
...
create policy artworks_read_reviewed on public.artwork_catalog for select to anon, authenticated using (enabled);
create policy mosaics_read_members on public.mosaics for select to authenticated using (private.is_mosaic_member(id));
create policy members_read_members on public.mosaic_members for select to authenticated using (private.is_mosaic_member(mosaic_id));
create policy activities_read_members on public.kindness_activities for select to authenticated using (private.is_mosaic_member(mosaic_id));
create policy contributions_read_boundary on public.kindness_contributions for select to authenticated using (
  private.is_mosaic_member(mosaic_id) and (participant_id = (select auth.uid()) or private.mosaic_revealed(mosaic_id))
);
create policy photos_read_boundary on public.event_photos for select to authenticated using (
  state = 'eligible' and private.is_mosaic_member(mosaic_id)
  and (photographer_id = (select auth.uid()) or private.mosaic_revealed(mosaic_id))
  and not exists (select 1 from public.user_blocks b where b.blocker_id = (select auth.uid()) and b.blocked_id = photographer_id)
);
create policy reports_read_self on public.event_photo_reports for select to authenticated using (reporter_id = (select auth.uid()));
create policy blocks_read_self on public.user_blocks for select to authenticated using (blocker_id = (select auth.uid()));

create or replace function public.v3_list_mosaics()
returns jsonb language sql stable security invoker set search_path = '' as $$
  select coalesce(jsonb_agg(private.summary_json(m) order by m.reveal_at), '[]'::jsonb)
  from public.mosaics m where private.is_mosaic_member(m.id);
$$;

create or replace function public.v3_resolve_invitation("invitationCode" text)
returns jsonb language plpgsql stable security definer set search_path = '' as $$
declare m public.mosaics;
begin
  select * into m from public.mosaics where invitation_code = upper(regexp_replace("invitationCode", '[^A-Za-z0-9]', '', 'g'));
  ...
  return jsonb_build_object(
    ...
    'description', m.description, 'revealAt', m.reveal_at, 'artwork', private.sealed_artwork_json(),
    'memberCount', (select count(*) from public.mosaic_members mm where mm.mosaic_id = m.id)
  );
end;
$$;
...
declare m public.mosaics; activity jsonb; code text;
begin
  if auth.uid() is null then raise exception 'account_required' using errcode = 'P0001'; end if;
  if not exists(select 1 from public.profiles where id = auth.uid()) then raise exception 'display_name_required' using errcode = 'P0001'; end if;
  ...
  insert into public.mosaics(creator_id, name, community_name, description, invitation_code, artwork_id, film_look_id, shot_limit, tile_goal, start_at, reveal_at)
  ...
  insert into public.mosaic_members(mosaic_id, profile_id, role) values (m.id, auth.uid(), 'creator');
  for activity in select value from jsonb_array_elements(payload->'activities') loop
    insert into public.kindness_activities(mosaic_id, title, purpose, sort_order)
    values (m.id, btrim(activity->>'title'), btrim(coalesce(activity->>'purpose','')), (activity->>'sortOrder')::int);
  end loop;
  return private.event_json(m);
end;
$$;
...
declare m public.mosaics;
begin
  if auth.uid() is null or not exists(select 1 from public.profiles where id = auth.uid()) then raise exception 'account_required' using errcode = 'P0001'; end if;
  select * into m from public.mosaics where invitation_code = upper(regexp_replace("invitationCode", '[^A-Za-z0-9]', '', 'g')) for update;
  if m.id is null then raise exception 'invalid_invitation' using errcode = 'P0001'; end if;
  if now() >= m.reveal_at then raise exception 'joining_closed' using errcode = 'P0001'; end if;
  insert into public.mosaic_members(mosaic_id, profile_id) values (m.id, auth.uid()) on conflict do nothing;
  return private.event_json(m);
end;
$$;
...
declare m public.mosaics;
begin
  if not private.is_mosaic_member("mosaicID") then raise exception 'not_a_member' using errcode = '42501'; end if;
  select * into strict m from public.mosaics where id = "mosaicID";
  return private.event_json(m);
end;
$$;
...
declare m public.mosaics;
begin
  select * into strict m from public.mosaics where id = "mosaicID" for update;
  ...
  update public.mosaics set name = btrim(v3_update_mosaic.name), description = btrim(v3_update_mosaic.description), updated_at = now()
...
begin
  if not private.is_mosaic_creator("mosaicID") then raise exception 'creator_required' using errcode = '42501'; end if;
  delete from public.mosaics where id = "mosaicID";
  return found;
end;
$$;
...
returns jsonb language plpgsql volatile security definer set search_path = '' as $$
declare m public.mosaics; position integer; c public.kindness_contributions; p public.profiles;
begin
  if not private.is_mosaic_member("mosaicID") then raise exception 'not_a_member' using errcode = '42501'; end if;
  select * into strict m from public.mosaics where id = "mosaicID" for update;
  if now() < m.start_at or now() >= m.reveal_at then raise exception 'contribution_closed' using errcode = 'P0001'; end if;
  if not exists(select 1 from public.kindness_activities a where a.id = "activityID" and a.mosaic_id = m.id) then raise exception 'invalid_activity' using errcode = 'P0001'; end if;
  if exists(select 1 from public.kindness_contributions x where x.mosaic_id = m.id and x.activity_id = "activityID" and x.participant_id = auth.uid()) then raise exception 'already_completed' using errcode = '23505'; end if;
  select candidate into position from generate_series(0, m.tile_goal - 1) candidate
  where not exists(select 1 from public.kindness_contributions used where used.mosaic_id = m.id and used.tile_position = candidate)
  order by candidate limit 1;
  if position is null then raise exception 'board_full' using errcode = 'P0001'; end if;
  insert into public.kindness_contributions(mosaic_id, activity_id, participant_id, tile_position, note)
...
returns jsonb language plpgsql volatile security definer set search_path = '' as $$
declare c public.kindness_contributions; m public.mosaics; p public.profiles;
begin
  select * into strict c from public.kindness_contributions where id = "contributionID" for update;
  select * into strict m from public.mosaics where id = c.mosaic_id;
  if c.participant_id <> auth.uid() then raise exception 'owner_required' using errcode = '42501'; end if;
  if now() >= m.reveal_at then raise exception 'mosaic_revealed' using errcode = 'P0001'; end if;
  update public.kindness_contributions set note = nullif(btrim(v3_update_contribution.note), ''), updated_at = now()
  where id = c.id returning * into c;
  select * into p from public.profiles where id = c.participant_id;
  return jsonb_build_object('id',c.id,'mosaicID',c.mosaic_id,'activityID',c.activity_id,'participantID',c.participant_id,
    'contributorDisplayName',p.display_name,'tilePosition',c.tile_position,'note',c.note,'createdAt',c.created_at,'updatedAt',c.updated_at,'isMine',true);
end;
$$;
...
returns boolean language plpgsql volatile security definer set search_path = '' as $$
declare c public.kindness_contributions; reveal_time timestamptz;
begin
  select * into strict c from public.kindness_contributions where id = "contributionID" for update;
  if c.participant_id <> auth.uid() then raise exception 'owner_required' using errcode = '42501'; end if;
  select reveal_at into reveal_time from public.mosaics where id = c.mosaic_id;
  if now() >= reveal_time then raise exception 'mosaic_revealed' using errcode = 'P0001'; end if;
  delete from public.kindness_contributions where id = c.id;
  return true;
end;
$$;
...
returns jsonb language plpgsql volatile security definer set search_path = '' as $$
declare m public.mosaics; path text; used integer; existing public.event_photos;
begin
  if not private.is_mosaic_member("mosaicID") then raise exception 'not_a_member' using errcode = '42501'; end if;
  select * into strict m from public.mosaics where id = "mosaicID" for update;
  ...
  select * into existing from public.event_photos where id="photoID";
  ...
  select count(*) into used from public.event_photos ep where ep.mosaic_id = m.id and ep.photographer_id = auth.uid() and ep.state in ('upload_pending','eligible');
  ...
  insert into public.event_photos(id,mosaic_id,photographer_id,storage_path,mime_type,byte_count,pixel_width,pixel_height,film_look_id,state)
...
returns jsonb language plpgsql volatile security definer set search_path = '' as $$
declare ep public.event_photos; p public.profiles;
begin
  select * into strict ep from public.event_photos where id = "photoID" for update;
  if ep.photographer_id <> auth.uid() then raise exception 'owner_required' using errcode = '42501'; end if;
  if private.mosaic_revealed(ep.mosaic_id) then raise exception 'photography_closed' using errcode = 'P0001'; end if;
  ...
  update public.event_photos set state='eligible', finalized_at=now() where id=ep.id returning * into ep;
  select * into p from public.profiles where id=ep.photographer_id;
...
declare ep public.event_photos;
begin
  select * into strict ep from public.event_photos where id="photoID";
  if ep.photographer_id <> auth.uid() then raise exception 'owner_required' using errcode='42501'; end if;
...
begin
  select * into strict ep from public.event_photos where id="photoID" for update;
  if ep.photographer_id <> auth.uid() then raise exception 'owner_required' using errcode='42501'; end if;
  delete from public.event_photos where id=ep.id;
  return true;
end;
$$;
...
begin
  select * into strict ep from public.event_photos where id="photoID" for update;
  if not private.is_mosaic_member(ep.mosaic_id) or not private.mosaic_revealed(ep.mosaic_id) then raise exception 'not_allowed' using errcode='42501'; end if;
  if ep.photographer_id = auth.uid() then raise exception 'cannot_report_self' using errcode='P0001'; end if;
  insert into public.event_photo_reports(photo_id,reporter_id,reason) values(ep.id,auth.uid(),btrim(reason)) on conflict do nothing;
  update public.event_photos set state='quarantined',quarantined_at=now() where id=ep.id;
  return true;
end;
$$;
...
begin
  ...
  insert into public.user_blocks(blocker_id,blocked_id) values(auth.uid(),"blockedID") on conflict do nothing;
  return true;
end;
$$;
...
returns boolean language plpgsql volatile security definer set search_path = '' as $$
begin delete from public.user_blocks where blocker_id=auth.uid() and blocked_id="blockedID"; return true; end;
$$;
...
returns jsonb language sql stable security definer set search_path = '' as $$
  ...
  from public.user_blocks b join public.profiles p on p.id=b.blocked_id where b.blocker_id=auth.uid();
$$;

create or replace function public.v3_release_artwork("mosaicID" uuid)
returns jsonb language plpgsql stable security definer set search_path = '' as $$
declare package private.artwork_reveal_packages;
begin
  if not private.is_mosaic_member("mosaicID") or not private.mosaic_revealed("mosaicID") then raise exception 'reveal_locked' using errcode='42501'; end if;
  select * into strict package from private.artwork_reveal_packages where mosaic_id="mosaicID";
...
returns jsonb language sql stable security definer set search_path = '' as $$
  ...
  join public.artwork_catalog a on a.id=m.artwork_id
  left join private.artwork_reveal_packages package on package.mosaic_id=m.id
...
returns boolean language plpgsql volatile security definer set search_path = '' as $$
begin
  if not exists(select 1 from public.mosaics where id="mosaicID") then
  ...
  insert into private.artwork_reveal_packages(mosaic_id,ciphertext_path,checksum,encryption_key,nonce)
...
create or replace function private.mark_due_mosaics_revealed()
returns integer language plpgsql volatile security definer set search_path = '' as $$
declare changed integer;
begin
  update public.mosaics set revealed_at=reveal_at,updated_at=now() where revealed_at is null and reveal_at<=now();
...
create policy event_photos_storage_insert on storage.objects for insert to authenticated with check (
  bucket_id='event-photos' and exists(
    select 1 from public.event_photos ep where ep.storage_path=name and ep.photographer_id=(select auth.uid())
      and ep.state='upload_pending' and not private.mosaic_revealed(ep.mosaic_id)
  )
);
create policy event_photos_storage_select on storage.objects for select to authenticated using (
  bucket_id='event-photos' and exists(
    select 1 from public.event_photos ep where ep.storage_path=name and ep.state='eligible' and private.is_mosaic_member(ep.mosaic_id)
      and (ep.photographer_id=(select auth.uid()) or private.mosaic_revealed(ep.mosaic_id))
      and not exists(select 1 from public.user_blocks b where b.blocker_id=(select auth.uid()) and b.blocked_id=ep.photographer_id)
  )
);
...
create policy artwork_packages_storage_select on storage.objects for select to authenticated using (
  bucket_id='artwork-reveal-packages' and exists(
    select 1 from private.artwork_reveal_packages package
    where package.ciphertext_path=name and private.is_mosaic_member(package.mosaic_id) and private.mosaic_revealed(package.mosaic_id)
  )
);

select cron.schedule('mosaic-v3-fixed-reveal','* * * * *',$$select private.mark_due_mosaics_revealed();$$);
