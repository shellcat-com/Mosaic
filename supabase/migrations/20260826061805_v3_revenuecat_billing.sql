...
create or replace function private.prevent_mosaic_access_source_change()
...
$$;
create trigger mosaics_immutable_access_source before update of access_source on public.mosaics
for each row execute function private.prevent_mosaic_access_source_change();
...
create table private.pass_redemptions (
  request_id uuid primary key,
  user_id uuid not null references public.profiles(id) on delete cascade,
  payload jsonb not null,
  state text not null check (state in ('reserved','completed','failed')),
  mosaic_id uuid references public.mosaics(id) on delete set null,
...
create policy billing_accounts_read_own on public.billing_accounts
  for select to authenticated using (user_id = (select auth.uid()));
revoke all on private.revenuecat_events, private.pass_redemptions from public, anon, authenticated;
...
returns boolean language sql stable security definer set search_path = '' as $$
  select coalesce((
    ...
    from public.billing_accounts b where b.user_id = target_user
  ), false);
$$;
...
begin
  ...
  insert into public.mosaic_members(mosaic_id,profile_id,role) values(m.id,target_user,'creator');
...
declare uid uuid := auth.uid(); source text; mosaic_id uuid; m public.mosaics;
begin
  ...
  if private.v3_plus_active(uid) then
    ...
    if exists(select 1 from public.mosaics x where x.creator_id=uid and now()<x.reveal_at) then
      raise exception 'free_creation_limit' using errcode='P0001';
    end if;
  end if;
  mosaic_id := private.v3_create_mosaic_for_user(payload,uid,source);
  select * into strict m from public.mosaics where id=mosaic_id;
...
declare uid uuid := auth.uid(); b public.billing_accounts;
begin
  if uid is null then raise exception 'account_required' using errcode='42501'; end if;
  select * into b from public.billing_accounts where user_id=uid;
...
begin
  ...
  insert into public.billing_accounts(user_id,revenuecat_app_user_id,plus_active,subscription_state,product_id,expires_at,will_renew,pass_balance,synchronized_at,reconciliation_required)
...
returns jsonb language plpgsql volatile security definer set search_path = '' as $$
declare existing private.pass_redemptions; account public.billing_accounts; mid uuid;
begin
  select * into existing from private.pass_redemptions where request_id="requestID";
  ...
  if private.v3_plus_active("userID") then
    ...
    insert into private.pass_redemptions(request_id,user_id,payload,state,mosaic_id,idempotency_key)
  ...
  select * into account from public.billing_accounts where user_id="userID" for update;
  if account.user_id is null or account.pass_balance<1 then raise exception 'insufficient_pass' using errcode='P0001'; end if;
  update public.billing_accounts set pass_balance=pass_balance-1,updated_at=now() where user_id="userID";
  insert into private.pass_redemptions(request_id,user_id,payload,state,idempotency_key)
...
returns uuid language plpgsql volatile security definer set search_path = '' as $$
declare redemption private.pass_redemptions; mid uuid;
begin
  select * into strict redemption from private.pass_redemptions where request_id="requestID" and user_id="userID" for update;
  ...
  update private.pass_redemptions set state='completed',mosaic_id=mid,updated_at=now() where request_id="requestID";
  return mid;
end;
$$;
...
begin
  update private.pass_redemptions set state='failed',failure_reason=left(reason,500),updated_at=now()
  where request_id="requestID" and user_id="userID" and state='reserved';
  get diagnostics changed = row_count;
  if changed=1 then
    update public.billing_accounts set pass_balance=pass_balance+1,reconciliation_required=true,updated_at=now() where user_id="userID";
  end if;
  return changed=1;
end;
$$;
...
declare prior private.revenuecat_events; uid uuid;
begin
  select * into prior from private.revenuecat_events where event_id="eventID";
  ...
  insert into private.revenuecat_events(event_id,payload_hash,event_type,app_user_id)
  ...
  if uid is not null then
    insert into public.billing_accounts(user_id,revenuecat_app_user_id,reconciliation_required)
    select uid,lower(uid::text),true where exists(select 1 from public.profiles where id=uid)
