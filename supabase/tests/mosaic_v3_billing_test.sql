...
select is((select access_source from public.mosaic_v3_mosaics where name='Plus'),'organizer_plus','existing premium access remains captured after expiry');
select throws_ok($$update public.mosaic_v3_mosaics set access_source='free' where name='Plus'$$,'P0001','mosaic_access_source_is_immutable','captured event access cannot be downgraded');
...
select is((select pass_balance from public.mosaic_v3_billing_accounts where user_id='86000000-0000-4000-8000-000000000001'),0,'reservation consumes the local mirrored PASS');
select is((public.v3_internal_begin_pass_redemption(
 '87000000-0000-4000-8000-000000000001','86000000-0000-4000-8000-000000000001',
 '{"name":"Pass","communityName":"Test","description":"","activities":[{"title":"Help","purpose":"","sortOrder":0}],"artworkID":"a0000000-0000-4000-8000-000000000001","filmLookID":"afterglow","shotLimit":24,"startAt":"2029-01-01T00:00:00Z","revealAt":"2030-01-01T00:00:00Z","goal":36}'::jsonb
...
select is(public.v3_internal_complete_pass_redemption('87000000-0000-4000-8000-000000000001','86000000-0000-4000-8000-000000000001'),(select mosaic_id from private.mosaic_v3_pass_redemptions where request_id='87000000-0000-4000-8000-000000000001'),'completion retry returns the same Mosaic');
