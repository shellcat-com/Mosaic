...
insert into public.mosaics(id,creator_id,name,community_name,description,invitation_code,artwork_id,film_look_id,shot_limit,tile_goal,start_at,reveal_at)
...
insert into public.mosaic_members(mosaic_id,profile_id,role) values
...
insert into public.kindness_activities(id,mosaic_id,title,purpose,sort_order) values
...
insert into public.kindness_contributions(id,mosaic_id,activity_id,participant_id,tile_position,note) values
...
insert into public.event_photos(id,mosaic_id,photographer_id,storage_path,mime_type,byte_count,pixel_width,pixel_height,film_look_id,state) values
...
insert into private.artwork_reveal_packages(mosaic_id,ciphertext_path,checksum,encryption_key,nonce) values
...
select is((select count(*)::int from public.mosaics),1,'member reads joined Mosaic');
select is((select count(*)::int from public.kindness_contributions),1,'member reads only own contribution before reveal');
select is((select count(*)::int from public.event_photos),1,'member reads only own photo before reveal');
...
select is((select count(*)::int from public.event_photos),0,'organizer has no special pre-reveal photo access');

select set_config('request.jwt.claims','{"sub":"91000000-0000-4000-8000-000000000004","role":"authenticated"}',true);
select is((select count(*)::int from public.mosaics),0,'nonmember cannot read event content');
...
reset role;
update public.mosaics set reveal_at=now()-interval '1 minute',revealed_at=now()-interval '1 minute' where id='92000000-0000-4000-8000-000000000001';
set local role authenticated;
select set_config('request.jwt.claims','{"sub":"91000000-0000-4000-8000-000000000002","role":"authenticated"}',true);
select is((select count(*)::int from public.kindness_contributions),2,'member reads all contributions after reveal');
select is((select count(*)::int from public.event_photos),2,'member reads all eligible photos after reveal');
...
reset role;
select is((select state from public.event_photos where id='95000000-0000-4000-8000-000000000001'),'quarantined','report immediately quarantines photo');

set local role authenticated;
select set_config('request.jwt.claims','{"sub":"91000000-0000-4000-8000-000000000002","role":"authenticated"}',true);
select is((select count(*)::int from public.event_photos),1,'quarantined photo is absent from gallery query');
select ok(public.v3_block_user('91000000-0000-4000-8000-000000000003'),'member can block contributor');
select is((select count(*)::int from public.event_photos),0,'blocking filters contributor photos');
select ok(public.v3_unblock_user('91000000-0000-4000-8000-000000000003'),'member can unblock contributor');
select is((select count(*)::int from public.event_photos),1,'unblocking restores eligible contributor photos');

select set_config('request.jwt.claims','{"sub":"91000000-0000-4000-8000-000000000001","role":"authenticated"}',true);
select ok(public.v3_delete_mosaic('92000000-0000-4000-8000-000000000001'),'creator may delete after reveal');
select is((select count(*)::int from public.mosaics where id='92000000-0000-4000-8000-000000000001'),0,'creator deletion cascades event');
