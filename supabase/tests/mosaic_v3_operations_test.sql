...
insert into public.kindness_activities(id,mosaic_id,title,purpose,sort_order)
select
...
select is(
  public.v3_update_contribution(
    (select id from public.kindness_contributions where activity_id='98000000-0000-4000-8000-000000000001'),
    'Edited note'
  )->>'note',
  'Edited note',
...
select is((select count(*)::int from public.kindness_contributions),0,'withdrawal releases its board position');

select public.v3_complete_activity(
  '97000000-0000-4000-8000-000000000001',id,null
)
from public.kindness_activities
order by sort_order
limit 9;
select is((select count(*)::int from public.kindness_contributions),9,'nine atomic claims fill a nine-tile board');
select is((select count(distinct tile_position)::int from public.kindness_contributions),9,'every claimed tile position is unique');
select ok((select min(tile_position)=0 and max(tile_position)=8 from public.kindness_contributions),'claimed positions stay inside board capacity');
select throws_ok(
...
reset role;
select is((select count(*)::int from public.event_photos),12,'twelve prepared photos consume all twelve shots');
set local role authenticated;
select set_config('request.jwt.claims','{"sub":"96000000-0000-4000-8000-000000000001","role":"authenticated"}',true);
select throws_ok(
...
reset role;
select is((select count(*)::int from public.event_photos),11,'active deletion restores one shot');
set local role authenticated;
select set_config('request.jwt.claims','{"sub":"96000000-0000-4000-8000-000000000001","role":"authenticated"}',true);
select ok(
  (public.v3_prepare_event_photo(
...
reset role;
select is((select count(*)::int from public.event_photos),12,'replacement returns the ledger to its shot limit');
insert into storage.objects(bucket_id,name,owner_id)
select 'event-photos',storage_path,photographer_id from public.event_photos
...
select is(private.mark_due_mosaics_revealed(),1,'scheduled processing marks a due Mosaic revealed');
select ok((select revealed_at is not null from public.mosaics where id='97000000-0000-4000-8000-000000000001'),'the fixed reveal timestamp is persisted');
set local role authenticated;
select set_config('request.jwt.claims','{"sub":"96000000-0000-4000-8000-000000000001","role":"authenticated"}',true);
select throws_ok(
